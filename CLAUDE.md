...
**Renderer / Electron UI changes: don't commit until the user has tested.** The renderer runs inside Electron's BrowserWindow, not Chrome, so the in-session browser MCP tools can't visually verify it. The default "boot the dev server and click around" instruction doesn't apply — instead, after edits to `src/renderer/**` (or anything else that affects the desktop app's UI), pause and ask the user to run `npm run dev` and check the change. Only commit once they've confirmed it looks right. This trumps the "commit before Done" rule above for UI work — wait for the visual sign-off first, then commit, then move the task.
...
- **Dependencies form a strict DAG.** `setDependencies` / `addDependency` reject cycles via a forward BFS through `task_dependencies`. "Resolved" detection (for `isReady` / `blockedBy`) matches columns whose name is `Done` OR `Archived` (case-insensitive) — archiving a blocker shouldn't un-resolve its dependents. The CLI's `--show-done` and `--show-archived` flags follow the same naming convention.
